...
public class MainItem
{
	...
	public static string strGetPercent(int value, sbyte isPer)
	{
		...
		{
			string text2 = text;
			text = text2 + value / 10 + "," + CRes.abs(value) % 10 + "%";
		}
		else if (isPer == 2)
		{
			string text2 = text;
			text = text2 + value / 100 + "," + CRes.abs(value) % 100 + "%";
		}
		else if (isPer == 10)
		{
			string text2 = text;
	...
	public static mVector SortVecItem(mVector vec)
	{
		...
		for (int i = 0; i < num - 1; i++)
		{
			...
			for (int j = i + 1; j < num; j++)
			{
				MainItem mainItem = (MainItem)vec.elementAt(j);
				if (mainItem.typeObject == 4 && ((MainItem)vec.elementAt(j)).ID < ((MainItem)vec.elementAt(num2)).ID)
				{
					num2 = j;
				}
			}
