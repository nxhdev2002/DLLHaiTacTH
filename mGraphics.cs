...
public class mGraphics
{
	...
	public void __drawRegion(Image image, int x0, int y0, int w, int h, int transform, float x, float y, int anchor)
	{
		...
		if (isClip)
		{
			...
			Rect rect = intersectRect(r2: new Rect(num12, num13, num14, num15), r1: r);
		...
		case 2:
			...
			if (isClip)
			{
				...
				else if ((float)(num12 + num14) < x + (float)w)
				{
					num8 = 0f - ((float)(num12 + num14) - x - (float)w);
				}
			}
			break;
		case 1:
		...
		Graphics.DrawTexture(new Rect(x + num3 + num16 + (float)num18, y + num4 + (float)num19 + num17, num * num7, num2 * (float)num9), image.texture, new Rect(((float)x0 + num3 + num8) / (float)image.texture.width, ((float)image.texture.height - num2 - ((float)y0 + num4)) / (float)image.texture.height, num / (float)image.texture.width, num2 / (float)image.texture.height), 0, 0, 0, 0);
		if (transform == 5 || transform == 6 || transform == 4 || transform == 7)
		{
