	...
	public void chatPopup(string text)
	{
		...
		if (GameCanvas.istest)
		{
			...
			if (text[0] == 'b')
			{
				...
				string[] array = mFont.split(text, ",");
				try
				{
					int num = int.Parse(array[0]);
					int num2 = int.Parse(array[1]);
					MainBuff mainBuff = new MainBuff((short)num);
					mainBuff.setTimeBuff(num2 * 1000);
					mainBuff.setData(0);
					mainBuff.setObjc(GameScreen.player);
					GameScreen.player.AddBuff(mainBuff);
				}
				catch (Exception)
				{
				}
			...
			else
			{
				try
				{
					...
					GameScreen.player.TestSkill((short)num3);
				}
				catch (Exception)
				{
				}
