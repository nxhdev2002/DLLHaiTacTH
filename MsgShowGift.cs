public class MsgShowGift : MsgDialog
{
	...
	private void paintShowVip(mGraphics g)
	{
		if (indexShow == 0)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				g.drawRegion(fraRuongVip.imgFrame, i % 3 * fraRuongVip.frameWidth / 3, 0, fraRuongVip.frameWidth / 3, fraRuongVip.frameHeight, 0, MotherCanvas.hw - fraRuongVip.frameWidth / 2 + i % 3 * (fraRuongVip.frameWidth / 3) + num2, MotherCanvas.hh - fraRuongVip.frameHeight / 2 + mShowRuongVip[num][i % 3] * 1 + num3, 0);
				if (i == 1)
				{
	...
	private void paintItemQuaySo(mGraphics g, int ypaint)
	{
		...
		{
			g.setColor(colorBorderVongQuay[0 + i % 2]);
